...
def loadBIRD(filename):
    ...
    with open(filename,"rt") as IN:
        for line in IN:
            ...
            ID=fields[3]; Preg=float(fields[8])
            byVariant[ID]=Preg
    return byVariant
...
#=========================================================================
if(len(sys.argv)!=5):
    exit(ProgramName.get()+" <data.txt> <chrom-or-dot> <threshold> <BIRD.out>\n")
(infile,chrom,threshold,BIRDfile)=sys.argv[1:]
threshold=int(threshold)

BIRD=loadBIRD(BIRDfile)
    ...
    for line in IN:
        ...
        if(True):
            print(ID,v,p,input1_alt,input1_ref,input2_alt,input2_ref,
                  input3_alt,input3_ref,significant,sep="\t",file=DEBUG)
