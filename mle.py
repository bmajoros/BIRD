...
while(True):
    ...
    thetas=[]
    numPools=len(variant.pools)
    for pool in variant.pools:
        if(not pool.isHetPool()): continue
        #theta=sum(pool.RNA)/sum(pool.DNA)
        refRNA=sum([x.ref for x in pool.RNA])
        altRNA=sum([x.alt for x in pool.RNA])
        refDNA=sum([x.ref for x in pool.DNA])
        altDNA=sum([x.alt for x in pool.DNA])
        q=float(altRNA+1)/float(altRNA+refRNA+2)
        p=float(altDNA+1)/float(altDNA+refDNA+2)
        theta=q/(1-q) / (p/(1-p))
        thetas.append(theta)
    ...
    CI_right=mean+1
    Preg=1
    print(variant.ID,round(mean,3),round(CI_left,3),round(CI_right,3),
          round(Preg,3),sep="\t")
